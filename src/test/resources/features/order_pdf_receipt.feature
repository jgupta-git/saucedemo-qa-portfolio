...
Feature: PDF order receipt
  ...
  Background:
    ...
    And I add "Sauce Labs Backpack" and "Sauce Labs Bike Light" to my cart
    And I complete checkout with valid information
    And the order should complete successfully

  @Scenario-1 @positive
  Scenario Outline: The on-screen checkout total matches the expected price
  ...
  @Scenario-2 @positive
  Scenario Outline: The downloaded PDF receipt matches the expected price and the order
